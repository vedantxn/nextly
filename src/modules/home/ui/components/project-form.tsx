...
export const ProjectForm = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    await createProject.mutateAsync({ ...values, model: "gpt-5.4" });
  };
        ...
        >
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 relative dark:text-white">
            ...
            <div className="flex items-center justify-between gap-x-2 relative">
              {/* Model label */}
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium backdrop-blur-md border shadow-sm bg-white/20 dark:bg-white/10 border-white/20 text-muted-foreground">
                <span className="size-3"><SiOpenai /></span>
                GPT-5.4
              </div>
